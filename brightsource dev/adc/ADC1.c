# include <ADC1.h>

void init_ADC1(void)
{
	SYSCTL->RCGCADC = (1<<1);        /*use ADC1 2  16 MHZ*/
	SYSCTL->RCGCGPIO = (1<<4);
	GPIOE->DIR &= ~(1<<1);
	GPIOE->AFSEL = (1<<1);          //pE1
	GPIOE->DEN  &=~(1<<1);  //disable  digital
	
	GPIOE->AMSEL = (1<<1);   //diable circuts with this  pin
	/*sample sequancer SS3 as it  one bit i/p */
	
	ADC1->ACTSS &= ~(1<<3);     //disblae sequncer
	
	ADC1->EMUX =(0xf<<12);   //tirger will start as sson as the adc enable  "continous sample "
	
	
	ADC1->SSMUX3 = 2;   //analgo input 
	
	ADC1->SSCTL3 = 0x6;  //enable intterupt 
	
	
	ADC1->ACTSS |= (1<<3); // enable the sequancer
}


uint32_t Get_ADC(void)
{
	volatile static uint32_t adcResult =0;
	adcResult = ADC1->SSFIFO3;
	return adcResult;
}
